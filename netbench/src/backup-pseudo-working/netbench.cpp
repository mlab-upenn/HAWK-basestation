...
int networkLoop()
{
  ...
  while(1) {
    ...
    // Convert to float
    convertDepthToFP((uint16_t *)depthbuf_undistort, depthfp);

    printf("Middle depth point = %f\n", 
	   ((float *)depthfp)[640*240 + 240]);
    ...
    // Generate cloud
    makePointCloud(rgbbuf_undistort, depthfp, cloud_msg);
...
int main(int argc, char ** argv)
{
  ...
  depth_pub = n.advertise<sensor_msgs::Image>("netbench/depth", 1);
  cloud_pub = n.advertise<pcl::PointCloud<pcl::PointXYZRGB> >("netbench/points", 1);
