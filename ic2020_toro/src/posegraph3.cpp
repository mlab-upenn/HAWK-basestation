...
bool TreePoseGraph3::loadNodes(vector<TreeNode>* nodes) 
{
    // clear all verts and edges
    clear();
    ...
    for (unsigned int i = 0; i < nodes->size(); i++)
    {
        ...
        p.z() = (*nodes)[i].p.z;
        p.yaw() = (*nodes)[i].p.yaw;
        p.pitch() = (*nodes)[i].p.pitch;
        p.roll() = (*nodes)[i].p.roll;
        TreePoseGraph3::Vertex* v = addVertex((*nodes)[i].id, p);
        if (v) {
    ...
    for (unsigned int i = 0; i < nodes->size(); i++)
    {
        for (unsigned int j = 0; j < (*nodes)[i].edges.size(); j++)
        {
            ...
            int id1 = (*nodes)[i].id;
            int id2 = (*nodes)[i].edges[j].id;
            TreePoseGraph3::Vertex* v1 = vertex(id1);
            TreePoseGraph3::Vertex* v2 = vertex(id2);
            
            Pose p;
            p.x() = (*nodes)[i].edges[j].p.x;
            p.y() = (*nodes)[i].edges[j].p.y;
            p.z() = (*nodes)[i].edges[j].p.z;
            p.yaw() = (*nodes)[i].edges[j].p.yaw;
            p.pitch() = (*nodes)[i].edges[j].p.pitch;
            p.roll() = (*nodes)[i].edges[j].p.roll;
...
bool TreePoseGraph3::saveNodes(std::vector<TreeNode>* nodes) {
    nodes->clear();
    ...
    for (TreePoseGraph3::VertexMap::iterator it = vertices.begin(); it != vertices.end(); it++) {
        ...
        temp.p.y = v->pose.y();
        temp.p.z = v->pose.z();
        temp.p.roll = v->pose.roll();
        temp.p.pitch = v->pose.pitch();
        temp.p.yaw = v->pose.yaw();
